...
A la fin d'une itération, un point était fait avec le client pour définir les objectifs pour la prochaine itération, le ``client'' ici étant notre tuteur, Julien Ponge.
...
Ce ticket peut ensuite être assigné à une personne et possède plusieurs états comme vu sur la figure~\ref{fig:workflow}.
\begin{figure}[H]
	\centering
...
La communication entre dépôts Git se faisant soit par le protocole ssh, https, ftp, rsync ou par le protocole git.
\begin{figure}[H]
	\centering
...
Toutes les réalisations faites sont ouvertes et disponibles à tout le monde : n'importe qui est libre de consulter, cloner ou de forker les projets présents sur GitHub.
Cependant, le droit de commit est restreint aux utilisateurs choisies.
Il est également possible de créer des dépôts privés sous conditions de payer un abonnement.

De nombreux projets utilisent GitHub comme plateforme, notamment rails, Rspec, scriptaculous...

Cette plateforme a été choisie par Julien Ponge pour héberger son dépôt Git.
Nous avons donc créés des comptes sur GitHub et avons forké le projet.
